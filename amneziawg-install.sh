...
install_awg_packages() {
    ...
    BASE_URL="https://github.com/ivanusevanton/awg-openwrt-apk/releases/download/"

    # проверка версии openwrt
    MAJOR_VERSION=$(echo "$VERSION" | cut -d '.' -f 1)

    if [ -z "$MAJOR_VERSION" ] || [ "$MAJOR_VERSION" -lt 25 ]; then
        printf "\033[31;1mОШИБКА: Этот скрипт поддерживает только OpenWrt 25.x и новее.\033[0m\n"
        printf "\033[31;1mВаша версия: %s\033[0m\n" "$VERSION"
        printf "\033[33;1mДля старых версий используйте другую версию скрипта или установите пакеты вручную.\033[0m\n"
        exit 1
    fi

    # используем только AWG 2.0
    AWG_VERSION="2.0"
    LUCI_PACKAGE_NAME="luci-proto-amneziawg"

    printf "\033[32;1mDetected OpenWrt version: %s → AWG protocol version: %s\033[0m\n" "$VERSION" "$AWG_VERSION"
    ...
    if apk info | grep -q "luci-proto-amneziawg\|luci-app-amneziawg"; then
    ...
    if [ "$AWG_VERSION" = "2.0" ]; then
        ...
        if [ "$INSTALL_RU_LANG" = "y" ] || [ "$INSTALL_RU_LANG" = "Y" ]; then
            if apk info | grep -q luci-i18n-amneziawg-ru; then
